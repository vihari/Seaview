...
/**
 * enableWrap part implemeted by Vihari Piratla(viharipiratla[at]gmail[dot]com)  
 * Note: The addon code is written in a way not supported with paging, use with care.
 * To make things efficient, rowSizes hash is computed as elements are instered into DOM.
 */
...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    function initializeRowSizes(){
	sumSizesInitialized = true;
	var i=0;
	sumSize = 0;
	while(true){
	    d = data.getItemByIdx(i);
	    ...
	    if(rowNum==data.getLength()-1)
		break;
	}
	$("div.grid-canvas").css("height",viewportHeight);
    }
	...
	if(options.enableWrap){
	    var rowNum = row;//data.getRowById(d.id);
	    var height=0;
	    	    
	    for(var i=0;i<rowNum;i++)
		height += rowSizes[data.getItem(i).id];
	    	    
	    stringArray.push("<div class='ui-widget-content " + rowCss + "' style='top:" + (height) + "px'>");
	}
	else{
	...
	//console.log(row+" htmllength: "+stringArray.length);
      var m = columns[cell];
      var d = getDataItem(row);
      var visibleRange = getVisibleRange();
      if(visibleRange.top==-1)
	visibleRange.top=0;
	 ...
	 stringArray.push("<div class='" + cellCss + "' style='white-space:normal;height:"+rowSizes[d.id]+"px;'>");
     }
	else{
    ...
    function resizeCanvas() {
      ...
      var visibleRange = getVisibleRange();
      numVisibleRows = visibleRange.bottom-visibleRange.top+1;//Math.ceil(viewportH / options.rowHeight);
      viewportW = parseFloat($.css($container[0], "width", true));
      if (!options.autoHeight) {
    ...
    function updateRowCount() {
      ...
      var oldH = h;
	if(options.enableWrap)
	    th = Math.max(maxSupportedCssHeight , viewportH - scrollbarDimensions.height);
	else
	...
	if(options.enableWrap){
	    ...
	    var i=0;
	    var height = 0;
	    if(!Object.keys(rowSizes).length){
		top = 0;bottom = 20;
		return {
		    top: top,
		    bottom: bottom,
	    ...
	    while(true){
		height += rowSizes[data.getItem(i).id];
		if(!topSet&&height>=(viewportTop+offset)){
		    topSet = true;top = i;
	        }
		if((height+rowSizes[data.getItem(i).id])>(viewportTop+offset+viewportH)){
		    bottom = i+1;break;
		}
    ...
    function updateRowPositions() {
	  ...
	  if(options.enableWrap){
	      var h = 0;
	      var arr = Object.keys(rowSizes);
	      for(var i=0;i<arr.length;i++){
		  if(typeof(data.getRowById(arr[i])) != 'undefined' )
		      h += rowSizes[arr[i]];
	      }
	      $canvas.css('height',h);
	  }
